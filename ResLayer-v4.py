...
class ImageViewer(QMainWindow):
    ...
    see_location_w_drill_lith=False
    ...
    base_folder=r'D:\Ameyem Office\Projects\Electric surveys\Easwar files\Hamirpur\\'   
    zoom_fact=1
    fileName=''
    pick_type='digitize'
    depth_pix_lims=[]
    ...
    def __init__(self):
        ...
        try:
            self.dvesdf,self.drill_dfs=load_pkl(self.base_folder+'dvesdf_datadf.pkl')
            self.dlith_dict=load_pkl(self.base_folder+'dlith_dict.pkl')

            self.dVES,self.dE,self.dN,self.dEle,self.dblock=self.dvesdf['VES No.'].values,self.dvesdf.Easting.values.astype(np.float), \
            self.dvesdf.Northing.values.astype(np.float), \
            str_array2floats(self.dvesdf.RL.values),self.dvesdf.Block.values
            
            self.dcoords=[[xi,yi] for xi,yi in zip(self.dE,self.dN)]

        except:
             self.dvesdf,self.drill_dfs,self.dlith_dict=[],[],[]
        try:
    ...
    def get_profile(self,locations4panel):
        profile=[]
        for loc in locations4panel:
            prof=self.vesdf[self.vesdf['VES No.']==loc][['Easting','Northing']].values.astype(np.float)
            profile.extend(prof)
            # print('Location coords: ',prof)
        return np.array(profile)
    def plotLocationPanel(self,locations4panel):
        (w,h)=4*len(locations4panel),10
        # print('Length datadfs 1st: ',len(self.data_dfs))
        ...
        profile=self.get_profile(locations4panel)
        # print('Location coords: ',profile)
        # profile=[]
        ...
        a.text(profile[0,0]-0.02, profile[0,1]-0.02, "L", ha="center", va="center", size=5,
                bbox=bbox_props)
        a.text(profile[-1,0], profile[-1,1], "R", ha="center", va="center", size=5,
                bbox=bbox_props)
    ...
    def decideWhere2go(self,event):
        if self.see_location_section:
            self.createNseeLocationSection(event)
        elif self.see_location_w_drill_lith:
            self.createNseeLocDrillLithSection(event)
        else:
            self.createVESprofile(event)
    def createNseeLocationSection(self,event): 
        locations=[]     
        local_coords=self.coords.copy()   
        locVES=list(self.VES.copy())
        # print(local_coords)
        selcoords=[]
        for i in range(3):   
            
            dist,idx=get_closePointidx([self.last_point.x(),self.last_point.y()], local_coords)
            locations.append(locVES[idx])
            popcoord=local_coords.pop(idx)
            selcoords.append(popcoord)
            locVES.pop(idx)
        self.listitem[0].setText('Selected VES No: ' +locations[0])
        selcoords=np.array(selcoords)
        indx=np.argsort(selcoords,axis=0)
        print('pops: ',selcoords)
        print('sorted: ',selcoords[indx[:,0]])
        print(locations,indx)
        locations=np.array(locations)[indx[:,0]]
        print(locations)
        self.plotLocationPanel(locations)
    
    def createNseeLocDrillLithSection(self,event):
        dist,idx=get_closePointidx([self.last_point.x(),self.last_point.y()], self.coords)
        location=self.VES[idx]
        dist,idx=get_closePointidx([self.last_point.x(),self.last_point.y()], self.dcoords)
        dlocation=self.dVES[idx]
        # figsize=4*2,10
        (w,h)=4*2,10
        myplt=plot_hibrid_VES_panel(self.vesdf,self.data_dfs,self.lith_dict,[location],
        self.dvesdf,self.drill_dfs,self.dlith_dict,[dlocation],figsize=(w,h))
        myplt.show()
    ...
    def createVESprofile(self,event):            
        ...
        if self.profiletobeselected:
            if self.VES[idx] not in [p for p,q in self.profile_ves]:
                self.profile_ves.append((self.VES[idx],self.coords[idx]))
    ...
    def makeProfile(self):
        ...
        self.see_location_section=False
        self.see_location_w_drill_lith=False
    def seeLocation(self):
        self.see_location_section=True
        self.profiletobeselected=False
        self.see_location_w_drill_lith=False
    def seeLocationWithLith(self):
        self.see_location_w_drill_lith=True
        self.profiletobeselected=False
        self.see_location_section=False
    ...
    def createActions(self):
        ...
        self.seeLocationAct=QAction("&See Location",self,triggered=self.seeLocation)
        self.seeLocationNdrillLithAct=QAction("&See Location with Lith",self,triggered=self.seeLocationWithLith)
    def createMenus(self):
        self.fileMenu = QMenu("&File", self)
        self.workMenu = QMenu("&Work", self)
        self.workMenu.addAction(self.makeProfileAct)
        self.workMenu.addAction(self.seeLocationAct)
        self.workMenu.addAction(self.seeLocationNdrillLithAct)
